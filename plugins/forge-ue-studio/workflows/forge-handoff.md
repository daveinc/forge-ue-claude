<!-- forge:workflow
name: handoff
consumes: forge.py exec status (active, blockers, orders, jobs), .forge/jobs/<work-order>/, .forge/runtime.json, editor state
...
One answer covers everything a handoff must record:

| In the payload | What to write into the handoff |
|---|---|
| `active` | Every live lease, its lane, its work order and its expiry |
| `blockers` | What stands between the returning session and more work, each with its `remedy` |
| `orders` | Which orders this session opened, and which never reached `ACCEPTED` or `REJECTED` |
| `jobs` | Every job folder on disk, and whether its `brief`, `packet`, `result` and `context` are present |
| `work_orders_path` | Where the returning session reads all of it from |

A `jobs` entry with a `brief` and no `result` is the interrupted work. Name it.

**Do not run `exec supervise` here** — supervision sweeps leases whose owner exited, and this session
has not exited. The point of a handoff is to record the lane state, not to recover it.
</step>
...
An order left at `DISPATCHED` — an `order_dispatched` blocker — is indistinguishable from work still
