...
4. Treat executable or plugin detection as `AVAILABLE_UNVERIFIED` until a safe end-to-end probe passes.
5. Probe each accepted route independently with known-good and known-bad controls. Never expose or persist credential values.
6. Distinguish actual generation/operation surfaces from planning or prompt-only skills. A model advertising vision/audio/tools is an input/tool capability, not proof of image, video, mesh or animation generation.
7. Emit capability contracts plus optional proposals. Compare each worker against the resident-host baseline for the exact task/complexity class and state context savings, benefit, effective cost, permissions, hardware fit, test, fallback, and affected workflows.
8. Pass proposals through `forge-capability-admin`. Do not install packages, download models, enable UE plugins, change PATH, write credentials, or edit the `.uproject` without separate explicit approval.
