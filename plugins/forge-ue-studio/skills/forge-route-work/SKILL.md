...
7. Apply the `required_tool_access` hard filter. Resolve every capability the packet declares through `python <forge-plugin-root>/scripts/forge.py mcp-status --project <project-root>`: a capability served by a typed tool route may be dispatched only to an agent that declares it and only while its contract reports the route bound. An unbound route is not a failure — dispatch the packet on the route's declared fallback and record which route was taken. Never dispatch a packet whose capability resolves to nothing.
8. Select isolation before dispatch. For a capability served by a typed tool route, the lane and isolation mode come from its registry row, not from a judgement call: a clean-base Git worktree for concurrent text/code writers, an LFS lock or project-exclusive lease for binary assets, and read-only isolation for reviewers. Never let two workers share an undeclared write surface.
9. Activate only the packet's required optional surfaces and acquire declared resources atomically. Every Unreal package writer shares the project super-lock; editor-open, editor-closed, and human routes are mutually exclusive.
