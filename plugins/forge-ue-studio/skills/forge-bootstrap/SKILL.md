...
description: Install or resume the project-local Forge overlay, inventory capabilities, delegate installation investigations, and stop at a fresh-task handoff. Use for first-time project setup or a partial adoption.
---
...
6. Compile only applicable jobs from [installation-waves.md](references/installation-waves.md). Record each job's canonical `FI-*` work order, objective, inputs, read/write scope, agent type, expected result and acceptance in `.forge/state/install-jobs.json` before dispatch.
