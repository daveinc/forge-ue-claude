...
2. If `.forge/state/lifecycle.json` is absent, apply the bundled project overlay with `scripts/forge.py install --project <root> --apply`. This is a reversible project-file installation and must not install packages, models, plugins, MCPs, or change the machine.
3. **STOP after first overlay installation.** Tell the user to open a fresh task in the project and run `$forge-bootstrap --resume`. Do not continue in the current task: the new `AGENTS.md`, project agents, state and skill surface must be loaded by a fresh host context.
4. On `--resume`, read `AGENTS.md`, `.forge/state/lifecycle.json`, `.forge/directives.md`, and the canonical packet registry. The lifecycle must name `$forge-bootstrap --resume` as `next_command`.
5. Apply lifecycle event `bootstrap-start`. Run deterministic Survey and Profile first; write `.forge/capabilities/detected.json`. Detection never grants qualification.
...
12. Apply lifecycle event `bootstrap-complete`. It must reject completion if the detected profile or bootstrap report is absent.
13. **STOP.** Present the exact persisted `next_command` (`$forge-init`) and require a fresh project task. Do not start design intake in the bootstrap task.
