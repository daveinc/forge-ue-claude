...
These jobs are executed by `forge-bootstrap`; this table is not merely advisory. The overlay is installed first, then the workflow stops so a fresh project session can load the rendered project instruction file, project-local agents, and directives. `forge-next` detects the incomplete bootstrap and routes to `forge-bootstrap --resume`; applicable jobs are compiled into `.forge/state/install-jobs.json`, dispatched by wave, independently verified, and summarized in `.forge/state/bootstrap-report.json`. A host that cannot dispatch agents records `DEGRADED_INLINE` rather than silently collapsing the jobs into the orchestrator.

| Wave | Agent job | Output |
|---:|---|---|
| 1 | Host/config investigator | OS, hardware, resident-host/GSD and write-boundary snapshot |
| 1 | Unreal investigator | Engine/project/plugin/build route inventory |
| 1 | Tool/model investigator | resident-host capabilities, Blender, MCPs, VibeUE, installed local runtimes, entitled services, approved remote APIs, and provider inventory |
...
| 2 | Provider evaluator | resident-host baseline plus per-task local-offload quality, context, cost, latency and contention scores |
...
GSD itself uses a deterministic bootstrap before these jobs: preview the exact pinned package, obtain separate approval for the external install, install its integration for the assigned host, inventory the resulting runtime/skills/agents, then verify Forge/GSD coexistence in a new task. Detection is not qualification; a failed or partial GSD install leaves the workflow visibly blocking instead of being silently assumed.
