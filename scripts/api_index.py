...
ERROR_REASON = {
    "API_INDEX_MISSING": "api_index_missing",
    "API_INDEX_UNKNOWN_TASK_CLASS": "api_index_unknown_task_class",
    "API_SYMBOL_ABSENT": "api_symbol_absent",
}
...
def api_call_names(
    ...
    registry = json.loads(
        (PLUGIN / "dependencies" / "route-registry.json").read_text(encoding="utf-8-sig")
    )
    provider = next(
        (row for row in registry.get("providers", []) if str(row.get("id")) == str(provider_id)),
        None,
    )
    if not provider:
        return []
